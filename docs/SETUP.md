...
최종 사용 흐름은 **캐릭터 선택 → n8n 대화 → 레시피 선택 → 음성 조리**입니다. 저장소에는 이 흐름에 사용한 웹 소스와 함께 초기 사진 검색·쇼핑 코드도 남아 있습니다. 각 파일의 용도는 [개발 이력과 코드 구분](DEVELOPMENT_HISTORY.md)을 참고하세요.
...
| 메신저형 대화 | `N8N_WEBHOOK_URL`, 별도로 구동하는 n8n 워크플로우와 그 안에서 사용하는 모델·도구 설정 |
...
Firebase 사진 업로드와 Naver 쇼핑 검색은 초기 구현입니다. 이를 최종 레시피 대화의 기능이나 필수 서비스 구성으로 소개하지 않습니다.

초기 화면을 별도로 실행하려면 해당 Firebase 설정과 Naver의 `CLIENT_ID`, `CLIENT_SECRET`이 필요합니다. 관련 코드의 위치는 [개발 이력](DEVELOPMENT_HISTORY.md)에 기록했습니다.

## 실행 확인 범위

- n8n 전체 워크플로우, FastAPI·음성 추론 서버, 음성 학습 데이터와 가중치는 별도로 준비해야 합니다.
