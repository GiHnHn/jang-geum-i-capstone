...
![초기 구현에서 재료 구매를 돕는 시연 화면](docs/images/demo-02-shopping-early.png)

개발 초기에 만들었던 재료 구매 보조 화면입니다. Naver 쇼핑 검색은 초기 부가 기능이며, 최종 메신저 대화·음성 조리의 주요 기능에서는 제외했습니다.
사진 업로드 후 GPT API로 레시피를 바로 조회하던 방식도 초기 구현입니다. 남아 있는 코드는 [개발 이력](docs/DEVELOPMENT_HISTORY.md)에 따로 정리했습니다.

## 시스템 구조

![n8n 메신저형 레시피 대화에서 음성 조리로 이어지는 장금이 시스템 구조](docs/images/system-architecture.png)

React 대화창에서 보낸 세션·캐릭터·메시지를 Express가 n8n에 전달합니다.
n8n은 자막 도구와 GPT API를 활용해 레시피에 답하고, 그 응답을 대화창에 표시합니다.
...
n8n 전체 워크플로우, 음성 추론 서버와 학습 가중치는 이 저장소에 포함하지 않았습니다.
초기 기능과 최종 흐름이 함께 남아 있는 파일은 [개발 이력과 코드 구분](docs/DEVELOPMENT_HISTORY.md)을 참고해 주세요.
